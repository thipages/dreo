...
/*! (c) Andrea Giammarchi - MIT */const un=(t=>(e,...n)=>new on(t,e,n))(!1);function pn(t,e){const n=0===t.length?"display:block":"display:none",r=t=>e(t.target.dataset.id);return un`
    ...
    `}input.value=An();const cn=function(t,e){const{width:n,height:r}=t.getBoundingClientRect(),s=Array(e).fill("").map(((t,e)=>e)),i=s.map((()=>document.createElement("canvas")));for(const e of i)e.width=n,e.height=r,e.setAttribute("style","\n    width: 100%;\n    height: 100%;\n    position: absolute;\n    top: 0;\n    left: 0;\n"),t.prepend(e);return{getContext:t=>i[t].getContext("2d"),getAll(){return s.map((t=>({ctx:this.getContext(t),clear:()=>this.clear(t)})))},getCanvas:t=>i[t].getContext("2d"),getDimension:()=>({width:n,height:r}),clear(t){this.getContext(t).clearRect(0,0,n,r)},clearAll(){for(const t of s)this.clear(t)},layerNum:e}}(cLayers,2);let ln=!1;t2.innerHTML=ie;let fn,dn=xn();function vn(t){const e=localStorage.getItem(t);if(!e)throw"ERROR";return dn=t,input.value=JSON.parse(e).code,En(),!0}function xn(){return(new Date).getTime()}function yn(t){t!==An()&&localStorage.setItem(dn,JSON.stringify({code:t,time:dn}))}function gn(t){const e=new Date(t);return new Intl.DateTimeFormat("fr-FR",{weekday:"short",year:"numeric",month:"short",day:"numeric",hour:"numeric",minute:"numeric",second:"numeric"}).format(e)}function mn(){hn(document.getElementById("dialog_list"),pn(function(){let t=[];for(const[e,n]of Object.entries(localStorage)){const{code:e,time:r}=JSON.parse(n),s=gn(1e3*(0|r));t.push({id:r,text:s})}return t}(),vn))}function wn(){return En(),input.addEventListener("keyup",(()=>{bn(input.value)||yn(input.value),En()})),btn_info.addEventListener("click",(()=>{dialog.showModal()})),btn_list.addEventListener("click",(()=>{mn(),dialog_list.showModal()})),btn_new.addEventListener("click",(()=>{bn(input.value)||yn(input.value),input.value="",fn&&fn.clear(),dn=xn(),cn.clearAll()})),"1.0"}function bn(t){return""===t.replace(/\s/g,"")}function En(){if(!ln){ln=!0,textError.innerText="",cn.clearAll();try{const{commands:t,error:e}=function(t){const e=[];let n=!1;try{const n=t.split("\n"),r=[];let s=!0;for(const[t,i]of n.entries())""!==i&&(s=Xt(i,t,r,e,s))}catch(t){n=t}return{commands:e,error:n}}(input.value);if(fn=te({layers:cn,commands:t}),e)throw e}catch(t){console.log(t),textError.innerText=t}finally{ln=!1}}}function An(){return"#L=1000\nr20\n-r36\n--a20\n--t10\n-t18\nd0\nr100\n-a228.6\n-d1\n-aL\n-a-L\n-d0\n-a-228.6\n-t3.6\n\nt-90\na400\nt90\n\na-35\nd1\nr36\n-a69.9\n-t10"}localStorage.clear(),wn();export{wn as run};
